...
class Propagator_pick():

    def propagate(self, model, particles,sliced, output_names=None):
        '''
        ...
        '''
        outputs = model(particles.params[:sliced,:])
        outputs = self._create_output_dictionary(outputs, output_names)
        return Particles(outputs, particles.log_likes[:sliced], particles.log_weights[:sliced])
